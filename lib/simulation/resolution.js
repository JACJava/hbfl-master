//  this is used once the simulation is complete and the records need to be sent to
//  SQS and Kinesis
console.log(Date().toLocaleString()+":  lib/simulation/resolution.js, initial comment")
...
function sendResults (race, results) {
  results.map((hamster, i) => {
    ...
    publish("this is a publish test")
  })

  const cleanResults = results.map((hamster, i) => {
    return { hamsterId: hamster.id, place: i + 1 }
  })
  console.log(Date().toLocaleString()+":  lib/simulation/resolution.js, by the kinesis send")

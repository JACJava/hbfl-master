...
const clientjak = new AWS.DynamoDB.DocumentClient()
TableName = 'hamsters'
...
function putResults (hamsterId, results) {
  return new Promise((resolve, reject) => {
    ...
    .then((hamster) => {
      ...
      console.log(Date().toLocaleString()+":  lib/data/lib/sqs.listener.js, Updating the item")

      clientjak.update(params, (err, data) => {
        if (err) reject(err)
        else resolve(data)
          console.log(Date().toLocaleString()+":  sqslistener.js, update function succeeded")
      })
